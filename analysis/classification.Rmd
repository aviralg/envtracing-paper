...
process_event_seq <- function(df) {
    ...
    mutate(event_seq = str_replace_all(event_seq, "R\\|", "R|")) %>%
    ...
    mutate(event_seq = str_replace_all(event_seq, "(\\?\\|)+", "?+|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "(L\\|)+", "L+|")) %>%
    ...
    mutate(event_seq = case_when(event_seq == "N Z !+" ~ "N Z !+ A*",
                                 ...
                                 ## methods
                                 event_seq == "N !+ L+" | event_seq == "N !+ L+ !+" | event_seq == "N L+ !+" ~ "N !* L+ !*",
                                 ## R6
                                 event_seq == "N A L+" | event_seq == "N A L+ @" | event_seq == "N !+ A L+" | event_seq == "N !+ A L+ @" | event_seq == "N !+ A @" ~ "N !* A L* @?",
                                 event_seq == "N @" | event_seq == "N @ A+" | event_seq == "N A+ @" | event_seq == "N A+ @ A+" ~ "N A* @ A*",
                                 event_seq == "N A+ L+" | event_seq == "N A+ L+ @" ~ "N A+ L+ @?",
                                 event_seq == "N !+ A+ L+" | event_seq == "N !+ A+ L+ @" ~ "N !+ A+ L+ @?",
                                 event_seq == "N [ A+ ]" | event_seq == "N [ ]" ~ "N [A*]",
                                 event_seq == "N @ !+" | event_seq == "N !+ @" ~ "N !* @ !*",
                                 event_seq == "N" | event_seq == "N A+" ~ "N A*",
                                 event_seq == "N !+" | event_seq == "N A+ !+" ~ "N A* !+",
                                 event_seq == "" | event_seq == "A+" ~ "N A*",
                                 TRUE ~ event_seq)) %>%
    mutate(event_seq = case_when(event_seq == "N A+ L+ @?" | event_seq == "N !* L+ !*" | event_seq == "N !+ A+ L+ @?" ~ "N !* A* L+ !* @?",
                                 TRUE ~ event_seq))
    
    df
}

user_explicit_event_seq <-
    all_env %>%
    filter(!core) %>%
    process_event_seq() %>%
    group_by(event_seq) %>%
...
datatable(user_explicit_event_seq)
...
core_explicit_event_seq <-
    all_env %>%
    filter(core & category == "Other") %>%
    process_event_seq() %>%
    mutate(event_seq = case_when(event_seq == "[ A+ ]" ~ "[A*]",
                                 event_seq == "[ ]" ~ "[A*]",
                                 TRUE ~ event_seq)) %>%
    group_by(event_seq) %>%
...
datatable(core_explicit_event_seq)
...
```{r echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
process_call_event_seq <- function(df) {
    df <-
    df %>%
    mutate(orig_seq = event_seq) %>%
    mutate(event_seq = str_replace_all(event_seq, "\\^\\d\\|", "|")) %>%
    ...
    mutate(event_seq = str_replace_all(event_seq, "Return\\|", "")) %>%
    mutate(event_seq = str_replace_all(event_seq, "parent.env_0\\|", "")) %>%
    mutate(event_seq = str_replace_all(event_seq, "as.environment_0\\|", "E|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "sys.frame_0\\|", "E|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "sys.frames_0\\|", "E|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "parent.frame_0\\|", "E|")) %>%
    ...
    mutate(event_seq = str_replace_all(event_seq, "parent.env<-_2\\|", "!|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "\\|parent.env<-_0\\|parent.env<-_1\\|", "|parent.env<-_1|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "environment_0\\|", "E|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "environment<-_1\\|", "!|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "parent.env<-_1\\|", "Z|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "(!\\|)+", "!+|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "(E\\|)+", "E+|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "(S\\|)+", "S+|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "\\|(A\\|)+", "|A+|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "\\|", " ")) %>%
    ...
    df <-
    df %>%
        mutate(event_seq = case_when(event_seq == "E+" | event_seq == "E+ A+" ~ "E+ A*",
...
## Core Package Environments
```{r echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
all_env %>%
filter(category == "Package") %>%
count(package_kind, wt = count, name = "count") %>%
arrange(desc(count)) %>%
mutate(cumperc = round(100 * cumsum(count)/ sum(count), 2)) %>%
slice(1:100) %>%
datatable()
...
process_event_seq() %>%
mutate(event_seq = str_replace_all(event_seq, "(A\\+ |!\\+ )+", "(A|!)+")) %>%
mutate(event_seq = str_replace_all(event_seq, fixed("(A|!)+A+"), "(A|!)+")) %>%
mutate(event_seq = str_replace_all(event_seq, fixed("(A|!)+!+"), "(A|!)+")) %>%
