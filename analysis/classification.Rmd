...
process_event_seq <- function(df) {
    ...
    mutate(event_seq = str_replace_all(event_seq, "EvalEntryIndirect\\|", "U|")) %>%
...
datatable(USER_EXPLICIT_ENV_EVENT_SEQ)
...
USER_EXPLICIT_ENV_EVAL <-
    USER_EXPLICIT_ENV %>%
    filter(!is.na(eval_category)) %>%
    count(eval_category, wt = count, name = "count")
...
datatable(USER_EXPLICIT_ENV_CLASS)
