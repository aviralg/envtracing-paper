...
CORE_EXPLICIT_OTHER_ENV <-
    all_env %>%
    filter(core) %>%
    filter(is.na(category) | category == "Other") %>%
    filter(str_starts(primary_source, fixed("base*$#$*lazyLoadDBexec"))    |
           str_starts(primary_source, fixed("base*$#$*loadNamespace"))     |
           str_starts(primary_source, fixed("base*$#$*package_load_file")) |
           str_starts(primary_source, fixed("base*$#$*dyn.load")))
...
cat("Other core explicit envs", sum(CORE_EXPLICIT_OTHER_ENV$count))
```
