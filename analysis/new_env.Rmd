...
process_event_seq <- function(df) {
    ...
    mutate(event_seq = str_replace_all(event_seq, "exists\\|", "?|")) %>%
    ...
    mutate(event_seq = str_replace_all(event_seq, "(list2env_0\\|list2env_1\\|)+", "L2E|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "(mget\\|)+", "mget+|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "\\|(get\\|)+", "|get+|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "env2list_1\\|", "E2L|")) %>%
    ...
    mutate(event_seq = str_replace_all(event_seq, "(\\?\\|)+", "?+|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "(🔒\\|)+", "🔒+|")) %>%
    mutate(event_seq = str_replace_all(event_seq, "(E2L\\|)+", "E2L+|")) %>%
...
new_env <-
    new_env %>%
    process_event_seq() %>%
    mutate(event_seq = case_when(event_seq == "N parent.env<-_1 !+" ~ "N parent.env<-_1 !+ get*",
                                 event_seq == "N parent.env<-_1 !+ get+" ~ "N parent.env<-_1 !+ get*",
                                 event_seq == "N E2L+" ~ "N E2L+ mget*",
                                 event_seq == "N E2L+ mget+" ~ "N E2L+ mget*",
                                 event_seq == "N L2E @" ~ "N L2E @ ?*",
                                 event_seq == "N L2E @ ?+" ~ "N L2E @ ?*",
                                 ## methods
                                 event_seq == "N !+ 🔒+" | event_seq == "N !+ 🔒+ !+" | event_seq == "N 🔒+ !+" ~ "N !* 🔒+ !*",
                                 ## R6
                                 event_seq == "N L2E 🔒+" | event_seq == "N L2E 🔒+ @" | event_seq == "N !+ L2E 🔒+" | event_seq == "N !+ L2E 🔒+ @" | event_seq == "N !+ L2E @" ~ "N !* L2E 🔒* @?",
                                 TRUE ~ event_seq))
new_env %>%
    filter(!implementation) %>%
