...
function and native code. 395.2K of these environments are created from user
packages. The remaining come from \emph{Core}.


%We observe 3.3M environments created using this
%function. The GNU R implementation (\base package) accounts for 87\% (2.9M) of
%these and packages contribute remaining 13\% (420.6K).
%
...
\emph{User}. Table~\ref{table:top_explicit_pack} shows the number of
...
\begin{table}[!h]
  \vspace{-3mm}
  \small
  \caption{Explicit Environment Packages} \label{table:top_explicit_pack}
  \centering
  \begin{tabular}{lrr}
    \toprule
...
Table~\ref{table:top_explicit_fun} shows the top ten functions. These functions
...
\begin{table}[!h]
  \vspace{-3mm}
  \small
  \caption{Top Explicit Environment Functions} \label{table:top_explicit_fun}
  \centering
  \begin{tabular}{lrr}
    \toprule
...
XXXX Table~\ref{table:explicit_env_attr} presents the class attributes attached
to environments by frequency and the package from which the attributes
originate.
\begin{table}[!h]
  \vspace{-3mm}
  \small
