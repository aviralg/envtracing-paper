...
function and native code. 6.3M of these environments come from core and 395.2K
from user packages.

\subsubsection{Core Explicit Environments}
We observe 6.3M explicit environments created from \emph{Core}. The package
loading mechanism accounts for 2.9M of these environments. 2.3M of these
environments come from a single \base package function, \lazyLoadDBexec, used
only internally. This function is responsible for loading a package's code from
a binary file and also for loading processed help file data. Since this is an
implementation detail, we ignore this for the rest of the discussion. 481K
functions come from the \code{loadNamespace} function of the \code{base}
package. This function is used for loading package namespaces. In the rest of
the discussion, we will ignore these 2.9M environments and discussion the
remaining 3.4M \emph{Core} environments.
...
\paragraph{What are environment life-cycles?} We can characterize the life-cycle
of these environments by summarizing the set of events that affect it.
Table~\ref{table:user_explicit_env_seq} shows the top 7 event sets, which
providing insights about 96.5\% of these environments. The events of interest
are: \texttt{A} for read, write and remove, \texttt{V} for evaluating code in
the environment, \texttt{L} for locking the environment or its bindings,
\texttt{@} for setting attributes, and, \texttt{!} for setting this environment
as the parent of another environment or lexical scope of a function.
