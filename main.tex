...
\begin{tikzpicture}
  ...
  \definecolor{installbg}{HTML}{D5A6BD}
  \definecolor{installol}{HTML}{666666}

  \definecolor{extractbg}{HTML}{FFF2CC}
  \definecolor{extractol}{HTML}{D6B656}
  
  \definecolor{tracebg}{HTML}{DAE8FC}
  \definecolor{traceol}{HTML}{6C8EBF}

  \definecolor{reducebg}{HTML}{D5E8D4}
  \definecolor{reduceol}{HTML}{82B366}

  \definecolor{combinebg}{HTML}{FFE6CC}
  \definecolor{combineol}{HTML}{D79B00}

  \definecolor{summarizebg}{HTML}{F8CECC}
  \definecolor{summarizeol}{HTML}{B85450}

  \definecolor{reportbg}{HTML}{E1D5E7}
  \definecolor{reportol}{HTML}{9673A6}

  \definecolor{groupbg}{HTML}{F5F5F5}

  \newcommand{\fstdtpt}[5]{
    \node [inner sep = 0, align = center, below right = 3mm of #5.south west, minimum width = 0.03 \textwidth] (#1) {
      \includegraphics[height=0.02 \textwidth]{#2}
    };
    \node [inner sep = 0, right = 0mm of #1, minimum width = 0.07 \textwidth, text width = 0.07 \textwidth, align = right] (#3) {\scriptsize #4}
  }

  \newcommand{\rstdtpt}[5]{
    \node [inner sep = 0, align = center, below = 3mm of #5, minimum width = 0.03 \textwidth] (#1) {
      \includegraphics[height=0.02 \textwidth]{#2}
    };
    \node [inner sep = 0, right = 0mm of #1, minimum width = 0.07 \textwidth, text width = 0.07 \textwidth, align = right] (#3) {\scriptsize #4}
  }

  \newcommand{\rstdtpth}[5]{
    \node [inner sep = 0, align = center, below = 3mm of #5, minimum width = 0.03 \textwidth] (#1) {
      \includegraphics[width=0.02 \textwidth]{#2}
    };
    \node [inner sep = 0, right = 0mm of #1, minimum width = 0.07 \textwidth, text width = 0.07 \textwidth, align = right] (#3) {\scriptsize #4}
  }

  \newcommand{\connect}[2]{\draw [line width=0.25mm, ->] (#1) edge (#2);}
  \tikzstyle{block} = [rectangle, minimum width=.08 \textwidth, minimum height=35pt]

  \newcommand{\fststage}[4]{\node [block, fill = #3] (#1) {\large #2};} % , draw = #4
  \newcommand{\rststage}[5]{\node [block, right = 0.030 \textwidth of #3, fill = #4] (#1) {\large #2};} % , draw = #5

  %%\node [rectangle, minimum width=.230 \textwidth, minimum height=35pt, fill = installbg] (corpus) {\large Corpus};
  %% Corpus
  \fststage{install}{\begin{tabular}{c}Install\\CRAN\end{tabular}}{installbg}{installol}
  %\fstdtpt{corpustimeicon}{clock-regular}{corpustimevalue}{\TimeTracingPipelineCorpus}{corpus};
  %\rstdtpt{corpuspackageicon}{cubes-solid}{corpuspackagevalue}{\CountTracingPipelineCorpusPackage}{corpustimeicon};
  %\rstdtpt{corpusscripticon}{r-project-brand}{corpusscriptvalue}{\CountTracingPipelineCorpusScript}{corpuspackageicon};

  %% Extract
  \rststage{extract}{\begin{tabular}{c}Extract\\Programs\end{tabular}}{install}{extractbg}{extractol}
  \connect{install}{extract}
  % \fstdtpt{mergetimeicon}{clock-regular}{mergetimevalue}{\TimeTracingPipelineMerge}{merge};
  % \rstdtpt{mergefileicon}{copy-regular}{mergefilevalue}{\CountTracingPipelineMergeFile}{mergetimeicon};
  % \rstdtpt{mergesizeicon}{database-solid}{mergesizevalue}{\SizeTracingPipelineMergeFile}{mergefileicon};

  %\draw ([xshift=9mm] 0.2 \textwidth,) -- ([xshift=9mm] 0.2 \textwidth,1);
    
  %% Trace
  \rststage{trace}{\begin{tabular}{c}Trace\end{tabular}}{extract}{tracebg}{traceol}
  \connect{extract}{trace}
  %\fstdtpt{tracetimeicon}{clock-regular}{tracetimevalue}{\TimeTracingPipelineTrace}{trace};
  %\rstdtpt{tracefileicon}{copy-regular}{tracefilevalue}{\CountTracingPipelineTraceFile}{tracetimeicon};
  %\rstdtpt{tracesizeicon}{database-solid}{tracesizevalue}{\SizeTracingPipelineTraceFile}{tracefileicon};

  %% Reduce
  \rststage{reduce}{\begin{tabular}{c}Reduce\end{tabular}}{trace}{reducebg}{reduceol}
  \connect{trace}{reduce}
  %\fstdtpt{reducetimeicon}{clock-regular}{reducetimevalue}{\TimeTracingPipelineReduce}{reduce};
  %\rstdtpt{reducefileicon}{copy-regular}{reducefilevalue}{\CountTracingPipelineReduceFile}{reducetimeicon};
  %\rstdtpt{reducesizeicon}{database-solid}{reducesizevalue}{\SizeTracingPipelineReduceFile}{reducefileicon};

  %% Combine
  \rststage{combine}{\begin{tabular}{c}Combine\end{tabular}}{reduce}{combinebg}{combineol}
  \connect{reduce}{combine}
  %\fstdtpt{combinetimeicon}{clock-regular}{combinetimevalue}{\TimeTracingPipelineCombine}{combine};
  %\rstdtpt{combinefileicon}{copy-regular}{combinefilevalue}{\CountTracingPipelineCombineFile}{combinetimeicon};
  %\rstdtpt{combinesizeicon}{database-solid}{combinesizevalue}{\SizeTracingPipelineCombineFile}{combinefileicon};

  %% Summarize
  \rststage{summarize}{\begin{tabular}{c}Summarize\end{tabular}}{combine}{summarizebg}{summarizeol}
  \connect{combine}{summarize}
  %\fstdtpt{summarizetimeicon}{clock-regular}{summarizetimevalue}{\TimeTracingPipelineSummarize}{summarize};
  %\rstdtpt{summarizefileicon}{copy-regular}{summarizefilevalue}{\CountTracingPipelineSummarizeFile}{summarizetimeicon};
  %\rstdtpt{summarizesizeicon}{database-solid}{summarizesizevalue}{\SizeTracingPipelineSummarizeFile}{summarizefileicon};

  %% Report
  \rststage{report}{\begin{tabular}{c}Report\end{tabular}}{summarize}{reportbg}{reportol}
  \connect{summarize}{report}
  %\fstdtpt{reporttimeicon}{clock-regular}{reporttimevalue}{\TimeTracingPipelineReport}{report};
  %\rstdtpt{reportcharticon}{chart-bar-regular}{reportchartvalue}{\CountTracingPipelineReportGraph}{reporttimeicon};
  % \rstdtpth{reportmacroicon}{latex-logo}{reportmacrovalue}{\CountTracingPipelineReportMacro}{reportcharticon};
  %{
  %  \node [inner sep = 0, align = center, below = 3.5mm of reportcharticon, minimum width = 0.03 \textwidth] (reportmacroicon) {
  %    {\scriptsize \LaTeX}
  %  };
  %  \node [inner sep = 0, right = 0mm of reportmacroicon, below = 4.3mm of reportchartvalue, minimum width = 0.07 \textwidth, text width = 0.07 \textwidth, align = right] (reportmacrovalue) {\scriptsize \CountTracingPipelineReportMacro};
  %};

  \begin{scope}[on background layer]
    \node[fit=(install) (extract), rectangle, fill=groupbg, draw=groupbg, fill opacity=0.8, label=above:\textbf{Corpus}] (corpus) {};
    \node[fit=(trace), rectangle, fill=groupbg, draw=groupbg, fill opacity=0.8, label=above:\textbf{Tracing}] (exectrace) {};
    \node[fit=(reduce) (report), rectangle, fill=groupbg, draw=groupbg, fill opacity=0.8, label=above:\textbf{Post-processing}] (mf) {};
  \end{scope}
  
  %\draw (-0.05 \textwidth, -25pt) -- (0.89\textwidth, -25pt);
  \draw (-0.05 \textwidth, -35pt) -- (0.87\textwidth, -35pt);

  \draw [line width=0.3mm] (-0.05 \textwidth, -55pt) -- (0.87\textwidth, -55pt);
\end{tikzpicture}
...
For this study, we assembled a corpus of the top 100 CRAN packages with highest
number of client packages. These 100 packages together have 11,786 clients.
Package \ggplot has the highest number of clients, 2,320, and package \vctrs has
the fewest, 108. These packages contribute 481.5K lines of R code and 1M lines
of native code. During execution, these 100 packages call functions from 186
other packages, so our evaluation also includes them. These extra packages have
478.2K lines of R code and 1.1M lines of native code.
...
In our corpus, we observed 1.2B environments, which makes them the second most
widely allocated values. Table~\ref{table:object_count_dist} shows the frequency
of other values for comparison. Promises are the most widely allocated values,
which is expected~\cite{oopsla19b}. Vectors of logicals and characters are more
frequent than integers, reals, and raw. Language objects, i.e, first-class
expressions, are used for metaprogramming. Lists are heterogeneous vectors.
Other objects such as S4 or externalptr are rare, and are presented together in
the \emph{Other} category.
