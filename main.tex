...
CRAN, based on the number of client packages. These packages have a total of XXX
client packages. XXX has the highest number of clients, XXX , and XXXX has the
fewest, XXXX. These 100 packages in turn depend on XXX extra packages, so we
also end up including those in our study. These XXX packages have XX lines of R
code and YY lines of native code. CRAN packages come equipped with runnable code
in the form of tests, examples, and long-form examples called vignettes. These
programs are extracted as independently executable scripts for evaluation by the
\experimentr library. Experiments demonstrate the use of a package's functions,
and vignettes illustrate a package's functionality with a larger example,
typically using data supplied with the package. Table
% \~ref{table:corpus}
presents the number and size of these scripts. Overall, \experimentr extracts
XXX programs with XXX lines of code.

The packages of our corpus have XXXX top-level functions. Table %~\ref{table:}
presents the distribution of functions across these packages. We observe taht
XXX packages have XY functions or less. As expected, there are few large
packages; only X packages have more than XXXX functions. We observed XXX calls
to these functions. Figure 2 shows the distribution of calls to these functions
called in the synthesis phase. XXXX\% functions are called more that ten times.
XXXX\% functions are called only once, leading to low coverage.

These functions have 185.9K parameter positions. 7.3\% functions have 0
parameters, 19.6\% have 1, and 5.0\% have over 10. There are 15 functions with
over 50 parameters that come from 10 packages. Of those, rpart.plot::
check.if.dot.arg.supported.by.rpart.rules has the highest, 122 parameters.
