...
\begin{figure*}[h!]
  ...
  \begin{tikzpicture}
    ...
    \node [block, fill = installbg]                                (install)   {\nodename{Install\\CRAN}};
    \node [block, right = \nodesep of install, fill = extractbg]   (extract)   {\nodename{Extract\\Programs}};
    \node [block, right = \nodesep of extract, fill = tracebg]     (trace)     {\nodename{Trace}};
    \node [block, right = \nodesep of trace, fill = reducebg]      (reduce)    {\nodename{Reduce}};
    \node [block, right = \nodesep of reduce, fill = combinebg]    (combine)   {\nodename{Combine}};
    \node [block, right = \nodesep of combine, fill = summarizebg] (summarize) {\nodename{Summarize}};
    \node [block, right = \nodesep of summarize, fill = reportbg]  (report)    {\nodename{Report}};
    ...
    \begin{scope}[on background layer]
      \node[fit=(install) (extract), rectangle, fill=groupbg, fill opacity=0.8, label=above:\textbf{Corpus}]          (corpus) {};
      \node[fit=(trace),             rectangle, fill=groupbg, fill opacity=0.8, label=above:\textbf{Tracing}]         (exectrace) {};
      \node[fit=(reduce) (report),   rectangle, fill=groupbg, fill opacity=0.8, label=above:\textbf{Post-processing}] (postprocessing) {};
    \end{scope}
  \end{tikzpicture}
