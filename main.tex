...
\begin{itemize}
\item[\textbf{S}:] This event happens in functions that use \code{substitute}.
...
\item[\textbf{X, A}:] These events happen to function environments whose
...
\item[\textbf{X, V, A}:] These events happen when an environment is extracted
...
\item[\textbf{X, S, V, A}] These events happen when an environment is used for
