...
\item[{\bf X,A}:] The environments are obtained as value and then used for
  accessing their bindings. For instance, \c{registerS3methods::assignWrapped}
  ...
  the parent environment extracted with \c{parent.frame}.
...
\noindent
\textbf{S:} This pattern comes from the \c{base::substitute} function. This
...
\noindent
\textbf{A, L, @:} These events occur on environments from the \c{later} and
\c{R6} packages . The \c{later} package uses environments as handles for
