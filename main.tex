...
The most important observation from this analysis is that most call environments
do not have to be explicit. One would expect an optimizing compiler to optimize
away accesses in these cases. Unfortunately, there are sufficient cases where
environments are accessed reflectively and the dynamic nature of R can make it
challenging to predict when this will happen. These environments can be passed
around and modified at will, making it hard to reason about the program from
that point onwards.
...
register a variety of events, implying that they rely on a significant
proportion of R's environment API.
...
\code{ggplot2} library implements \code{ggproto}, a prototype based OO system.
The \code{ggproto} objects are explicit envivronments with the
\code{``ggproto''} class attribute. The \code{XML} package creates an
...
state for implementing dataframe indexing.
...
locked to prevent modifcation. The environments are then assigned the class
...
local scopes are frequently occurring events for these environments. They are
rarely used for building formulas.
...
In our future work, we plan to dig deeper in the data we have gathered and
